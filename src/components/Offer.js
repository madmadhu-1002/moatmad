  ...
  return (
    <div>
        <Container fluid className='mb-5' style={{ padding: "0px 32px" }}>
          <Row style={{ padding: "0px 18px 200px 20px" }}>
            ...
            {offers.map((offer, index) => (
              <Col lg={6} key={offer.id} className='' style={{ boxShadow: "0 1px 3px #0000001f, 0 1px 2px #0000003d", height: "330px", paddingLeft: index % 2 === 0 ? '0px' : '15px', paddingRight: index % 2 !== 0 ? '0px' : '15px' }}>
                <img src={offer.image} style={{ objectFit: "cover" }} height={'100%'} width={'100%'} alt={offer.image_alt || offer.title_en} />
                <div className="m-5" dangerouslySetInnerHTML={{ __html: offer.summary_en }} />
              </Col>
            ))}
          </Row>
        </Container>
